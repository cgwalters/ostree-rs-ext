...
async fn handle_layer_progress_print(
    ...
    pb.set_style(style.template("{prefix} {pos:>3}% [{bar:20}] ({eta}) {msg}"));
    loop {
        tokio::select! {
            // Always handle byte-level changes first to ensure the
            ...
            r = layer_bytes.changed() => {
                ...
                if let Some(bytes) = &*bytes {
                    let percentage = ((100f64 * bytes.fetched as f64) / bytes.total as f64) as u64;
